...
class Config:
    ...
    # scheduler / news / formatting (optional)
    sched_every_seconds: int
    news_feeds: str
    news_keywords: str
    format_lang: str
...
def load_config() -> Config:
    ...
    if not guild_id:
        raise RuntimeError("DISCORD_GUILD_ID missing (must be a number)")
    ...
    return Config(
        ...
        sched_every_seconds=int(_str("SCHED_EVERY_SECONDS", "3600")),
        news_feeds=_str("NEWS_FEEDS", ""),
        news_keywords=_str("NEWS_KEYWORDS", ""),
        format_lang=_str("FORMAT_LANG", "ru"),
    )
