...
class Config:
    ...
    telegram_allowed_chat_id: int | None  # <--- ВАЖНО: ограничение на 1 чат/группу

    # Bridge (опционально)
    bridge_discord_channel_id: int | None
    bridge_telegram_chat_id: int | None

    # Остальное (опционально, оставил как было)
...
def load_config() -> Config:
    ...
    return Config(
        ...
        telegram_allowed_chat_id=_int("TELEGRAM_ALLOWED_CHAT_ID"),
